<template>
  <el-menu :default-active="activeMenu" mode="horizontal" :ellipsis="false" @select="handleSelect">
    <template v-for="(item, index) in topMenus">
      <el-menu-item v-if="index < Number(visibleNumber)" :key="index" :style="{ '--theme': theme }" :index="item.path"><svg-icon :icon-class="item.meta?.icon" /> {{ item.meta?.title }}</el-menu-item>
    </template>
    ...
    <el-sub-menu v-if="topMenus.length > Number(visibleNumber)" :style="{ '--theme': theme }" index="more">
      ...
      <template v-for="(item, index) in topMenus">
        <el-menu-item v-if="index >= Number(visibleNumber)" :key="index" :index="item.path"><svg-icon :icon-class="item.meta!.icon" /> {{ item.meta!.title }}</el-menu-item>
      </template>
    </el-sub-menu>
  </el-menu>
</template>
