<template>
  <div>
    ...
    <el-table border v-loading="loading" :data="tableList" @selectionChange="handleSelectionChange">
    ...
    <!-- 预览界面 -->
    <el-dialog :title="preview.title" v-model="preview.open" width="80%" top="5vh" append-to-body draggable custom-class="scrollbar">
...
async function handleGenTable(row: any) {
  ...
  if (row.genType === '1') {
    ...
    proxy.$download.zip('/tool/gen/batchGenCode?tables=' + tbNames, 'code.zip')
  }
}
/** 同步数据库操作 */
...
async function handlePreview(row: any) {
  const res: any = await previewTable(row.tableId)
